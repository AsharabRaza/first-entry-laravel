                                    ...
                                    <table class="table table-bordered text-nowrap border-bottom w-100" id="responsive-datatable">
                                        ...
                                        @if(count($winners) > 0)
                                            @foreach($winners as $winner)
                                                @php
                                                    $entry = \App\Models\Entry::select('*')->where('id',$winner->entry_id)->first();
                                                @endphp
                                                <tr>
                                                    ...
                                                    @php
                                                        $g_row = \App\Models\Entry::select('*')->where('id',$entry->guest_id)->first();
                                                    @endphp
                                                    <td class="text-center" style="vertical-align: middle;">
                                                        @if ($g_row && $g_row->g_first_name != '')
                                                            {{ $g_row->g_first_name }}
                                                    ...
                                                    <td>{{ formatted_date($winner->created_at) }}</td>
                                                    @php
                                                    ...
                                                    <td class="text-center align-middle action">
                                                        @if($entry->has_parent == 0)
                                                            <div class="btn-group align-top">
                                                                <button class="btn btn-sm btn-primary badge remove_winners" type="button" id="remove_winners" data-lid="{{ $winner->lottery_id }}" data-id="{{ $winner->id }}" data-eid="{{ $entry->id }}" data-gid="{{ $entry->guest_id }}" data-gid2="{{ $g_id }}"><i class="bi bi-trash-fill"></i><div class="spinner-border spinner-border-sm" style="display: none;place-content: center;align-items: center;width: 10px;height: 10px;" role="status"></div></button>
                                                            </div>
                                                        @else
                                                            <span style="display: block;text-align: center;font-size: 60%;font-style: italic;font-weight: bold;">GUEST</span>
                                                        @endif
...
<script>
    $('#download_table').click(function(){
        $("#responsive-datatable").table2csv({
            filename: 'Winners - <?php echo htmlspecialchars('ALL');?> (<?php echo date('M d, Y', strtotime($start_datetime));?>).csv',
